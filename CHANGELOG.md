...
- **An over-long object name is rejected instead of terminating the backend** — `bw_read_metadata_tables` built its SQL from the name as given, so a name wider than the key column it is compared against (30 characters for a provider, 32 for a transformation) hit the DataPreview service with a truncation dump and reached the caller as an opaque HTTP 500. The width is checked first, and the answer now names the problem.
- **`bw_read_metadata_tables` reads DTPs again** — resolving the DTP's transformation compared RSBKDTP's own source/target tokens against RSTRAN's TLOGO columns. For a DataSource-fed DTP that token is `DTASRC`, six characters against a CHAR(4) column, so the DataPreview service terminated with a truncation dump instead of returning rows; where it did not dump, `DTASRC` simply never matched `RSDS` and the transformation was reported as not found. The comparison now uses the TLOGO columns RSBKDTP carries alongside them. Found while tracing a load chain on a BW 7.5 system.
