...
- **`bw_system_profile`** — reports what the connected system is (BW/4HANA vs classic BW, from the system's own `b4hanamode` flag), which REST endpoint groups it publishes and therefore which tool groups work on it, and whether the two preconditions hold: `Accept`-header handling and access to the ADT DataPreview service. Call it first on an unknown system, or when tools fail with 404 or 406.
- **`bw_read_metadata_tables`** — read-only fallback that reads an object straight from its metadata tables for the types a system publishes no REST resource for: `TRFN` (including start, end, expert and field routine source), `DTPA`, and the classic providers `ODSO`, `CUBE` and `MPRO`, which no release exposes over REST.
