...
- **Writing BW objects works on a classic BW 7.5 system** — the platform filter added in 1.5.0 hides the tools whose REST resource a classic release does not publish, and everything it left visible counted as working there by omission. That held for the reads, which were verified. It did not hold for the writes: they had never been run against a classic system, and the first run found that essentially every one of them failed. Three causes, each of which produced a message pointing somewhere else.

  **The media type was resolved against the wrong key.** The discovery document is read at startup and overrides the hardcoded defaults, downgrades included — but it keys on the collection name, and that name is not stable across releases: a classic system publishes InfoObjects as `infoobject` where BW/4HANA publishes them as `iobj`. The discovered `iobj-v1_8_0` was therefore filed under a key nothing looked up, the hardcoded `v2_2_0` stayed, and the lock was rejected with HTTP 415 naming a content type mismatch. The reads were unaffected because the read path sent a list of versions rather than one — an InfoObject-specific workaround that existed for exactly this reason, alongside a second, InfoArea-specific one. Both are gone: the collection spelling is mapped onto the key the server addresses, and one shared function derives the version range wherever a list is wanted.

  **The lock ran under a session type the backend refuses.** A classic backend validates a lock handle against the ADT session that took it, and keeps that session alive only when the lock asks for one. Without it — or with `stateful_enqueue`, which BW/4HANA accepts — the lock returned a handle and the very next request was refused with `ExceptionResourceInvalidLockHandle`, "lock handle for object … could not be created". The message names the enqueue, the cause is the session. On a classic release every lock now runs as plain `stateful`. **On BW/4HANA nothing changes**: the caller's request is passed through untouched, and so is the behaviour where no platform verdict exists yet.

  **A request between lock and write dropped the session.** The reusable query components call `/sap/bc/adt/cts/transportchecks` after locking. A request that declares no session type ends a stateful one, which invalidated the handle the following write was about to use — the same 423, one step later. The transport check now carries the session type of the lock it sits inside.

  **The InfoObject write had to leave the lock's session entirely.** This one produced no error at all: after the media type fix the lock succeeded, the backend accepted both POST and PUT, answered "object changed successfully" — and applied nothing. A key figure body produced a characteristic, CHAR(5) with ALPHA; a PUT that changed only the description did not arrive either. An ADT communication trace of Eclipse BWMT creating the same InfoObject on 7.5 showed why: BWMT uses its stateful enqueue session for the lock and the unlock and for nothing else, and sends every read, write and activation on a session of its own. Over JCo that separation is free and therefore invisible; over HTTP it has to be asked for. The InfoObject write now runs in its own session — deliberately not every type, because an InfoSource PUT applies from the lock session and, sent from elsewhere, leaves the activation checking the state from before it. The create also carries the full object there: BW/4HANA ignores the create body and takes its values from the PUT that follows, while a classic release reads it and rejects a stub outright.

  Verified object by object on SAP_BASIS 750, each write read back afterwards: InfoAreas, InfoObjects (characteristic and key figure, with their length, texts and InfoArea as given), aDSOs including field, key and settings changes, InfoSources, CompositeProviders, queries with all five update tools, variables, restricted and calculated key figures, reusable structures, aggregation levels, move, unlock and delete. `bw_set_query_roles` and `bw_create_transport_task` were expected to be untestable there and were not — the system has the roles and a modifiable request.

  **The DataSource resource pinned its version too.** `bw_set_datasource_fields` and `bw_change_datasource_delta` sent `rsds-v1_1_0` in the PUT rather than the version discovery resolved, and a backend serving `v1_0_0` rejected both with HTTP 415 — the same defect as the InfoObject media type, in a second hardcoded place. Found by exercising them against a DataSource nothing uses, which `bw_xref` identifies.

  **One object type is still refused**: `bw_create_datasource`. The create POST is rejected with `ExceptionResourceInvalidLockHandle` although the lock just issued that handle. Two ADT traces of Eclipse BWMT creating a DataSource on 7.5 were compared call by call and every difference they showed has been adopted — the package reference in the body, a real application component, the transport check and the name validation between lock and write, and each combination of lock and write session. Request, headers, content type and body now match the traced ones and the lock answers exactly as it does there, so what remains is not visible in the protocol. Create the DataSource in BWMT or the SAP GUI; reading it, selecting its fields and changing its delta process all work from here afterwards.

- **`bw_list_remote_entities` answered HTTP 404 on a classic release** — the value help it reads is published there, at a different address and with different parameter names: `is/values/hanaentity` with `pattern` and `maxrows`, where BW/4HANA has `rsdsint/values/hanaentity` with `searchPattern` and `resultSize`. The answer is shaped differently too — a column catalog and rows of bare values, positional, rather than one named element plus attributes per row — so the tool reads both, taking the column list rather than a fixed position to decide which cell is the entity name. `bw_preview_datasource` also goes to `rsdsint` and still answers 404 there; its classic address is not known.

- **`bw_change_datasource_delta` could set a delta process but never remove one** — the empty string the tool documents for "remove the delta process" was validated against the list of admissible values, which by definition never contains it, so every attempt came back "not admissible". A delta could be set and not taken off again. Not release-specific: equally broken on BW/4HANA.

- **`HTTP 403 CSRF token validation failed`, seemingly at random** — two requests issued in parallel on one client both found no token and both fetched one; the second fetch starts a new session and invalidates the token the first is about to send. The failure then surfaced at the next write, which had nothing to do with either read. It looked transient because it depended on which fetch answered first, and it liked to appear on the first write of a session because platform detection issues exactly such a pair before the first tool call. A token fetch is now shared by every caller that asks while it runs.

- **`bw_get_infoarea` returns fields instead of raw XML on a classic system** — the resource answers in XML there where BW/4HANA answers in JSON, and the tool handed the whole document back under `raw` for the caller to parse. Both forms are now read into the same four fields.

### Added

- **A variable can be read back and corrected** — `bw_get_variable` and `bw_update_variable` close the last gap among the reusable query building blocks, which until now could only be created. The reason is verification, not symmetry: the modelling API accepts an enum literal it does not know, stores its default and reports the object as consistent, and `bw_create_variable` passes five such literals. Without a way to read the object back, a processing type that quietly fell back to the default was invisible — reading the variable through its query does not help, because a query resolves the technical name of a variable reference and never its definition.

  `bw_update_variable` changes description, ready-for-input, entry requirement, selection type and processing type while the UID stays the same, so references from queries, CKFs and structures survive. The correction path before was delete and recreate, which BW refuses once anything references the variable — and deleting the query does not take its reusable components with it, so a variable created wrong stayed in the system out of reach. The reference characteristic and the variable type are rejected instead of sent: the backend accepts such a write, answers "consistent" and keeps the old value, which is exactly the silent coercion these two tools exist to expose.

  The Accept negotiation for the variable resource existed twice, in the read path and in the create path; it is now one definition, in the same shape as the query and RKF resources. Verified on both platforms — BW/4HANA and SAP_BASIS 750 — by creating a variable, reading every enum back, changing it, and confirming that a query using it still carries the same reference afterwards.

- **`bw_system_profile` states what each write tool does on a classic system** — the tool listed what the platform hides and was silent about the rest, which reads as "everything still offered works". It now prints a verdict per write tool: verified, blocked with what to do instead, or still to test with the missing precondition. Generated from one table in the source, and a test fails if a write tool is added without one, so the tool surface and the statement about it cannot drift apart.

### Notes

- The count of tools hidden on a classic release is **45**, not the 44 named when the filter shipped; `bw_delete_request` joined them in the same version. `docs/BW75-SUPPORT.md` carries the corrected number and a new section on the writes.

## [1.5.0] — 2026-09-17

### Added

- **Reusable calculated key figures can be created and changed** — `bw_create_ckf` takes the formula as the operator/operand tree `bw_get_ckf` hands back, so a definition read from one system can be written to another unchanged. `bw_update_ckf` takes either a whole new formula or targeted operations, which is what makes the common case — add another summand to a sum — possible without knowing or rebuilding the expression that is already there.
- **Reusable key figure structures can be created and changed** — `bw_create_structure` and `bw_update_structure` build the object that reporting queries embed as an axis, with members referencing a reusable CKF or RKF or a basic key figure. A change at the structure reaches every query that embeds it, which is the point of the object and the reason the tools sit at the structure rather than at one query that happens to use it.
- **A restricted key figure can be corrected in place** — `bw_update_rkf` changes description, base key figure and restrictions while the UID stays the same, so references from CKFs, structures and queries survive. The only correction before was delete and recreate, which BW refuses once the RKF is referenced anywhere — the state a wrong restriction tends to be discovered in.
- **Load requests can be deleted** — `bw_delete_request` removes requests from an InfoProvider together with the data they brought in and their entry in request management. BW demands this before several follow-up steps, above all switching a DTP from delta to full extraction, which it refuses while delta requests remain in the target; `all_requests=true` clears a provider for exactly that case. The two kinds of request are handled differently, as BW handles them: a load request in the inbound queue is deleted, an activation request is rolled back — and a rollback also undoes every later activation and the load request beneath it, which the response spells out rather than leaving to be discovered. Not to be confused with selective deletion by condition, which removes rows and leaves the request standing. Asynchronous: a successful call starts the deletion, `bw_list_requests` confirms it.
- **An analyst role, for asking questions of the data** — the HTTP transport had two scopes, and a business user who should only run a query was handed the whole modelling surface: a hundred tools to pick from, most of them about transformations, DTPs and process chains. The new `analyst` scope carries reporting and nothing else: run a query or a provider directly, read the characteristic values to filter by, find what there is to ask (`bw_search`, the roles assigned to the caller) and understand what is being asked (query, calculated and restricted key figures, structures, InfoObjects, the providers behind them), plus the load history for "is today's data in yet". Fourteen tools — the size is the point, because a client carrying all 105 reaches for the wrong one far more often than one carrying fourteen, and a test enforces the count so that growth is a decision rather than an accident.

  **Purely additive: `read` is unchanged and still admits everything it did**, including `bw_query_data`. The analyst set is a strict subset of it, and the role exists to give a business user a smaller client, not to take anything from a reader. `bw_preview_datasource` is left out although it returns rows — it samples source-system data while modelling an extraction, an ETL task rather than reporting — as is `bw_list_contents`, which walks the repository tree by object type folder and is the modeller's way in. Both stay available under `read`. The BTP role collections follow: **BW MCP Analyst** next to **BW MCP Reader** and **BW MCP Developer**; a caller may hold several. stdio is unaffected, as it has no scopes at all.
- **Characteristics can be modelled for values with lower case letters or umlauts** — `bw_create_infoobject` takes `lower_case`, and `bw_update_infoobject` can change the flag afterwards. Without it, a characteristic rejects any value outside the permitted character set (RSKC), and it does so at the worst possible moment: the load reports success — a push answers HTTP 204 and the record is physically in the inbound table — and only the request activation fails. The flag is the single object-local switch that turns that check off, and it was not reachable through either tool, which left a whole class of characteristics (identifiers, language keys, names) unmodellable end to end. Changing it on a characteristic already used by a provider that holds data is accepted, so a request that failed activation on such a value can simply be activated again.
- **`bw_get_request` reads the log of each process step** — a process step that runs under its own TSN, a request activation above all, keeps its messages in its own process log, and only the request-level log was read. A failed activation therefore appeared as a red step with the reason nowhere in the output: the message list held the (successful) messages of the load itself, and diagnosis ended at "finished with errors". The per-step logs are read alongside it now, so the actual message — the value, the characteristic and the message number — is in the same answer. On a request with more steps than the per-call cap, the steps that did not finish green are read.
- **The tool surface follows the platform** — one instance of this server fronts one BW system, and that system is either BW/4HANA or a classic release. Until now the model saw the full BW/4HANA surface either way: against a BW 7.5 system it called `bw_list_requests`, `bw_get_transformation` and `bw_list_process_chain_runs` first, because their descriptions promise they work, collected an ICF HTTP 404 from each — delivered as a complete HTML logon error page — and only then found the route that does work. A hint in the prompt does not compete with a tool list, so the tool list is what changed. The platform is detected once per process from the system's own `bw.b4hanamode` flag and its discovery document, and `tools/list` then offers only what that system can answer: a tool addressing a `/sap/bw/modeling` collection is offered exactly where the collection is published, so the system decides and not a hardcoded release list. On a 7.5 system 44 tools drop out; on BW/4HANA the list is unchanged. A tool called anyway — a client holding a stale list — is answered with the reason instead of a request to BW. `BW_PLATFORM=auto|classic|bw4` overrides the verdict: `classic` forces it where detection cannot run, `bw4` switches the filter off as the escape hatch. Detection never fails closed — an unidentifiable system gets the full surface and a warning in the log.
- **Every hidden tool names the route that works** — a filtered tool list answers half the question: `bw_get_transformation` is simply gone, and nothing says that the same rules and routine sources are in the metadata tables. The substitute call is therefore spelled out in two places from one table — in the message a client with a stale tool list gets back, and in the instructions the server sends at handshake time, narrowed to what this particular system actually hides. Deliberately **not** a redirect inside the tools themselves: the REST API and `bw_read_metadata_tables` run against different backends — the latter is direct table access through ADT DataPreview — and separate tools keep that boundary where it can be governed, so an installation that does not grant ADT simply withholds that one tool. A tool that quietly switched backends would dissolve the boundary, and it would fail silently where a hidden tool fails visibly.
- **`bw_read_metadata_tables` reads analysis processes** (`object_type="ANPR"`) — the APD is the one object type here that closes no gap of a particular release: no release ever published a REST resource for it and BW/4HANA dropped it altogether, so the metadata table is the only route on any platform, as with data slices. The whole definition is a single XML document in `RSANT_PROCESS.XML` — a column typed STRG, which the DataPreview service does return, worth stating because it drops other long-text types and it would be easy to conclude the definition is out of reach. It comes back as the process it describes: nodes in execution order with the object each source reads and each target writes, the ranges of a filter, the formulas of a formula node and the ABAP of a routine node. The order is computed from the `<MAPPINGS>` block and never from the document, which lists nodes in no particular order — a process whose target element comes first would otherwise read as writing before it reads — and a node fed by two branches is listed after both, with its predecessors named. A node type the catalogue does not know is still shown with its category, taken from the tag prefix, and its attributes; dropping it would report a process that reads from nowhere. Verified against every analysis process on a reference system, the shipped SAP content included: 20 node types across 15 processes, the largest with 45 nodes.
- **`bw_read_metadata_tables` reads process chain runs** (`object_type="RSPCLOG"`) — the chain *definition* already had a route, its *execution history* had none: on BW/4HANA the monitoring comes from the RV_C_PCM* OData services, which a classic release does not register at all, so `bw_list_process_chain_runs`, `bw_get_process_chain_run_detail` and `bw_list_process_chain_last_status` are hidden there and "why did the chain fail last night" had no answer short of the SAP GUI. One object type covers all three questions, told apart by what the name turns out to be rather than by its shape — a 25-character value is looked up as a log id first, because RSPC_CHAIN allows names that long too. A chain name returns the run history newest first, ordered in the statement so that the row cap takes the most recent runs and not an arbitrary slice, plus the steps of the newest run, which is almost always the question being asked. A log id returns that run's steps with status, start, duration and the process variant behind each, and names the steps that did not end green. A pattern such as `Z*` returns the last status of every matching chain, read one chain at a time so the answer is complete rather than capped. Status codes come back as the raw letter with its colour and meaning, the way the load history does it, and a step whose recorded status differs from its actual one — what a manual repair leaves behind — says so instead of picking one. The message log of a failed step is an application log and stays out of reach through table access; the output says that rather than appearing to have looked.
- **`bw_read_metadata_tables` reads the planning objects** (`object_type="PLSE"`, `"PLSQ"`, `"PLCR"`, `"PLDS"`) — a classic release publishes `alvl` but none of the three planning resources, so `bw_get_planning_function`, `bw_get_planning_sequence` and `bw_get_planning_properties` are hidden there and the BW-IP mechanics of a system — the part a migration assessment is actually about — had no route at all: FOX formulas, characteristic relationships and data slices were reachable only through the SAP GUI. They now come from the metadata tables. A planning function returns its function type with the exit class behind it, the aggregation level and its InfoArea, the characteristic usage, the conditions, and the full parameter tree — assembled from the type's parameter *declaration*, so the values nest and are labelled the way the function type defines them instead of arriving as a flat list of names. Variable references are resolved to their technical names rather than printed as a 25-character UID that reads like a characteristic value, and a FOX formula comes back as source code instead of one table row per line. A sequence returns its steps in execution order with aggregation level, function and filter. `PLCR` returns the planning properties of an InfoProvider — key date, max combinations, save strategy — together with the characteristic relationships and their validity ranges. **Data slices (`PLDS`) have no REST resource on any release**, so that type is not a classic-only substitute but the only route to them on either platform. `PLCR` and `PLDS` are keyed by the InfoProvider, not by the aggregation level. No new tool: four object types on the existing one, and the three hidden tools name their substitute like every other. Verified against a grown customer estate as well as the reference system: a FOX formula is **not** stored one row per source line but as a character stream cut into fixed-width chunks whose line breaks sit inside the values, so it is reassembled before it is shown — printed per chunk it came back with words cut in half. Conditions are grouped by rule, because a function with two rules restricted to different value types reads as one function with four mutually exclusive conditions otherwise, and the chunk numbers of a formula are left out rather than repeated once per chunk.
- **`bw_read_metadata_tables` reads aDSOs** (`object_type="ADSO"`) — the load history of an aDSO had no route at all on a classic release: the tool refused the type, and `bw_list_requests` needs the BW/4HANA manage API that release does not have. It returns the history only, because the `adso` REST resource exists everywhere and `bw_get_adso` already reads structure and settings. The output names its source: `RSSTATMANPART` is the classic request store and is empty by design on BW/4HANA, where requests live in `RSPMREQUEST` — so an empty section there is not a statement about the provider.

### Improved

- **`bw_get_process_chain_run_detail` lost the whole answer when the message log failed** — steps and messages were fetched with `Promise.all`, so a failure in either took both with it, and the gateway in front of these services answers with HTTP 500 every so often under load (observed on a customer system, where the same URL succeeded three times in a row moments later and the failure moved between the run list and the run detail). The two reads are sequential now, the message log is best effort — a run whose log cannot be read still returns its steps, and says why the log is missing — and every OData read in the monitoring tools is retried once on a server error. The message list is also no longer printed in full: one run of a real chain carries hundreds of messages (824 on the run this was written against), nearly all of them progress notes, so errors and warnings are shown and the rest is reported as a count — identical messages collapsed to one line with an occurrence count, because the same warning repeats once per affected object, and without the long text, which on these messages is the SAPscript help document converted to HTML and restates the message in a few hundred characters of escaped markup. The run that prompted this went from 824 messages with their long texts to two lines.
- **A duration longer than an hour is reported in hours** — the load history and the chain runs share one formatter, and it stopped at minutes: a chain that waited overnight for a cancelled step to be repeated came back as `1107m 29s`. Same duration, but nobody converts that in their head, and it is the first number a reader looks at.
- **`bw_get_planning_function` returned an incomplete definition, and said nothing about it** — five defects in the PLSE reader, each of which produced a plausible answer rather than an error, found by reading the same objects through `bw_read_metadata_tables` and comparing. A parameter written as a self-closing tag (a key figure selection, typically) was taken for an opening one, so the search for its closing tag ran into the *next* parameter's and swallowed everything in between — on a copy or distribution function that is the whole from/to table, which is the entire content of the function. The restrictions of a data selection live in a `fieldSelectionSet` rather than in a `selectionRange` and were not read at all, so those tables came back empty even where they survived. `exclude="true"` was dropped, which turns "everything but the unassigned value" into "exactly the unassigned value" — the reader stated the opposite of the model. A function type that supports several rules writes one `<condition>` block per rule, and only the first was read, so half a function was presented as the whole one; rules are now reported separately, and named where there is more than one. And XML entities in values were passed through undecoded, so FOX source containing `==>` came back as `==&gt;`, which is not the code that runs. Regression-tested against an offline copy of a real response, and the two routes now agree fact for fact — including the FOX source character for character — on every function, sequence and provider they were compared on.
- **An ICF error page never reaches the chat** — every `/sap/bw4/…` path answers with the full HTML "Logon Error Message" page on a classic system, thousands of bytes that told the model nothing. An HTML error body is now replaced by the sentence it means, naming the path and pointing at `bw_read_metadata_tables`; ADT exception documents are passed through unchanged, because callers parse them. Every HTTP failure in the client goes through one place now instead of twenty.
- **`bw_system_profile` no longer reports a resource as working because the probe failed** — its reporting check answered "implemented" for every error except the one it recognised, so an unrelated HTTP 403 (a CSRF token rotated by the DataPreview probe just above it) had a classic system report query reporting as OK — in the same output that listed `bw_query_data` as unavailable. The published collection decides first now, which is the same signal the tool filter uses, so the two cannot contradict each other; the probe runs only where the resource is published, and an error that never reached the handler is reported as UNCLEAR rather than as a verdict. The check also keyed on the wrong collection (`query`, the query *definition* resource, which classic BW publishes as well) instead of `reporting`.
- **`bw_system_profile` reuses the detected profile** instead of measuring again, so it cannot contradict `tools/list`, and it lists every tool the platform hides together with the reason. Its endpoint groups accept both spellings of the InfoObject collection — classic advertises `infoobject`, BW/4HANA `iobj`, and the resource is `iobj` on both — which had it reporting a working group as unavailable on either platform.
- **Metadata table reads survive a rotated CSRF token** — the token is fetched from the BW modeling service and spent on the ADT DataPreview service, and the backend rotates it out from under the client: reading modeling metadata first was enough to have the next read refused with HTTP 403. These statements have no side effects, so one retry with a fresh token is safe, and the two statements of a load history read no longer run in parallel against the same token. A dropped connection is retried once for the same reason: a read that needs several statements keeps a connection busy long enough for the server to recycle it, and the resulting `ECONNRESET` reached the caller as a stack trace rather than as an answer.

### Fixed

- **An MCP client that asks for no scopes no longer breaks the hosted instance** — the OAuth metadata did not advertise `scopes_supported`, so a client requested none. XSUAA grants them anyway, and Copilot's older auth code then falls back to the JWT `scope` claim — an array on XSUAA, a string everywhere else — and fails with `scope.split is not a function`. Fixed upstream in vscode#325344, but the Eclipse Language Server lags behind, so the server advertises the scopes itself. That also narrows the verifier's accepted scopes from the framework default set to the three this server defines.
- **Setting a unit or currency source no longer destroys a neighbouring key figure mapping** — `bw_update_transformation` with `unit_source_field` removed the *first* rule targeting the unit/currency field so it could fold it into a combined rule. In a transformation that already carries combined amount+currency rules, that first rule belongs to another amount, so every call silently dropped one neighbouring mapping (observed on a BW/4HANA system: two rules lost per transformation, with the call reporting success). Only a rule whose sole target is the unit/currency field is folded now.
- **A formula over more than one source field is modellable** — `bw_update_transformation` with `rule_type="formula"` wired up exactly one source, and `additional_source_fields` was accepted and ignored. A formula rule is only parseable once every operand is registered as a source of the rule, so the typical derived flag — `IF ( A = … AND B = … AND C > … , 'X' , '' )` — was written without complaint and then failed activation with "syntax error in formula", a message that points at the text while the text is fine. The rule is now built with one `<source>`/`<input>` pair per operand, and the operands are read out of the expression itself and resolved against the source segment (a custom characteristic appears as `/BIC/NAME` there, a standard one without its leading zero), so a multi-operand formula works without listing its fields. A field named by the caller that the source segment does not have is refused before anything is written, instead of being stored as a rule that cannot be activated.
- **The formula text of an existing formula rule can be changed** — the same call on a rule that was already `StepFormula` reported success, echoed the new text back, and stored the old one: rule content was only ever written while converting a rule from another step type, and a rule that needed no conversion was left alone. Changing a formula is the most common case there is, and the answer was indistinguishable from a real change. The rule is rebuilt now whatever step type it has. The two conversions that key on the source step type — to a field routine and to a constant — no longer report a conversion they did not perform: a step type they cannot convert is named in the response instead, and a constant rule that is already a constant has its value replaced rather than silently kept.
- **A formula that comes back from BW can be handed straight back in** — `bw_get_transformation` printed the formula exactly as stored, XML entities and all, and feeding that text into an update escaped it a second time (`&gt;` became `&amp;gt;`). The formula is shown decoded now, and an update accepts either spelling and escapes it exactly once.
