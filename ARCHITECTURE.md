...
**Model reads are stateless:** `freshRead()` — the pre-lock and verification read every tool uses, and the read-modify-write base of the DTP tools — goes out through a brand-new client with `forceCacheUpdate=true` and `X-sap-adt-sessiontype: stateless`. The same per-session buffer that pins state after a write also *accumulates*: the DTP `/m` handler returns the children of `<semanticGroup>`, `<temporaryStorage>` and `<programFlow>` n-fold on the n-th read of the same DTP within one stateful context, and behind a proxy that injects the `sap-contextid` it holds into every request a "fresh" client still lands in that one context. A stateless read gets a context of its own that ends with the request, so it returns the database state exactly once and leaves nothing behind. `bw_get_dtp` additionally collapses repeated `groupField` entries and says so, and `applySemanticGroup` never sends them back n-fold.
...
└── tools/
    ...
    ├── metadata_tables.ts # bw_read_metadata_tables — reads TRFN, DTPA, the classic providers
    │                     # (ODSO, CUBE, MPRO) from their metadata tables via ADT DataPreview
    │                     # and RSPC, for systems that publish no REST resource for them; a cube
    │                     # or DSO also reports its load history (RSSTATMANPART + RSBKREQUEST),
    │                     # a chain its steps and dependencies resolved into execution order
